...
static size_t range_start;
static size_t range_end;
...
lv_obj_t * lv_rlottie_create_from_raw_range(lv_obj_t * parent, lv_coord_t width, lv_coord_t height, const char * rlottie_desc,const size_t start,const size_t end)
{
...
static void lv_rlottie_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    //修改的代码
    if(range_start>0){
        rlottie->current_frame=range_start;
        rlottie->range_start=range_start;
    }
    if(range_end>0){
        rlottie->total_frames=range_end;
        rlottie->range_end=range_end;
    }
...
static void next_frame_task_cb(lv_timer_t * t)
{
    ...
    if((rlottie->play_ctrl & LV_RLOTTIE_CTRL_PAUSE) == LV_RLOTTIE_CTRL_PAUSE) {
        ...
        if((rlottie->play_ctrl & LV_RLOTTIE_CTRL_BACKWARD) == LV_RLOTTIE_CTRL_BACKWARD) {
            if(rlottie->current_frame > rlottie->range_start)
            ...
            if(rlottie->current_frame < rlottie->total_frames)
                ...
                if((rlottie->play_ctrl & LV_RLOTTIE_CTRL_LOOP) == LV_RLOTTIE_CTRL_LOOP)
                    rlottie->current_frame = rlottie->range_start;
                else {
                    lv_event_send(obj, LV_EVENT_READY, NULL);
                    lv_timer_pause(t);
                    return;
